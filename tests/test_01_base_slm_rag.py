...
def log_interaction(query, response, sources, latency):
    ...
    with open("results/test_01_rag_log.txt", "a", encoding="utf-8") as f:
...
def run_interactive_test():
    ...
    def process_query(q):
        ...
        if best_distance <= EXACT_MATCH_THRESHOLD:
            ...
            ans, num_tokens = generate_answer(q, chunks, model, tokenizer, device, system_prompt=EXACT_RAG_SYSTEM_PROMPT)
        elif best_distance <= STRONG_MATCH_THRESHOLD:
            rag_info += f"\n[Case: STRONG - Injecting {', '.join(source_ids)}]"
            ans, num_tokens = generate_answer(q, chunks, model, tokenizer, device, system_prompt=RAG_SYSTEM_PROMPT)
            ...
            ans, num_tokens = generate_answer(q, [], model, tokenizer, device, system_prompt=BASE_SYSTEM_PROMPT)
